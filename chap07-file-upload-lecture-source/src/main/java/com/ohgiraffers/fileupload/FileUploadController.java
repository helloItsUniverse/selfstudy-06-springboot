...
public class FileUploadController {
    ...
    /* 설명. buil 된 파일 업로드 할 경로를 가져오기 위해 ResourceLoader 의존성 주입받기 */
    @Autowired
    ...
    public String singleFileUpload(@RequestParam MultipartFile singleFile,
        ...
        /* 설명. build 경로의 static 에 있는 파일 업로드 할 곳의 경로를 받아온다.(미리 경로에 해당하는 디렉토리 생성 및 빌드 확인) */
        ...
        /* 설명. 사용가가 넘긴 파일을 확인하고, rename 해 보자.(우리는 자바의 UUID 클래스를 활용해서 rename 함) */
        ...
        try {
            ...
            /* 설명. BL 이 성공하면 화면의 재료를 RedirectAttributes 로 담는다. (flashAttribute) */
        ...
        // F5 눌렀을 때 연속적인 insert 를 방지하기 위해 redirect
...
//        System.out.println("multiFileDescription = " + multiFileDescription);
        ...
        /* 설명. 사용자가 올린 파일들을 다 rename 하고 저장하는 작업 및 DB로 전달하기 위한 List 에 쌓기 */
        List<Map<String, String>> files = new ArrayList<>();    // DB 에 저장할 값들을 지닌 List
        List<String> saveFiles = new ArrayList<>();             // 화면 단에서 img 태그가 참조할 정적 리소스 경로(src 경로)
        ...
        try {
            ...
            /* 설명. List<Map<String, String>> files 로 쌓인 업로드된 파일들을 찾아 일일이 다시 지운다. */
