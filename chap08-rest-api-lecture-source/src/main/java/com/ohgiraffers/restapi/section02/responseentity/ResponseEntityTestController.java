...
@RestController
@RequestMapping("/entity")  // 나중엔 이게 Micro Service 의 이름
public class ResponseEntityTestController {
    ...
    /* 설명. 2. builder 를 활용한 메소드 체이닝 방식으로 ResponseEntity 객체 만들기 (요즘 유행) */
    ...
    /* 설명.
     *  기존에 우리가 배웠던 @RequestParam 과 달리 json 문자열이 핸들러 메소드로 넘어올 때는 @RequestBody 를 붙이고 받아내야 한다.
     *  json 문자열의 각 property별로 받을 수도 있지만 한번에 하나의 타입으로 다 받아낼 때는 커맨드 객체(UserDTO)를 활용해야 하며
     *  json 문자열의 property명과 일치해야 한다.
     *  .
     *  {
     *      "id": "user01"
     *  }
    ...
    public ResponseEntity<?> registUser(@RequestBody UserDTO newUser) {
        // ? 는 바디에 실어서 넣을 값이 없을 때
        System.out.println("newUser = " + newUser);

        /* 설명. AUTO_INCREMENT 개념을 컬렉션 마지막에 있는 회원 번호 +1 로 해서 newUser 에 추가하기 */
    ...
    public ResponseEntity<?> modifyUser(@RequestBody UserDTO modifyInfo, @PathVariable int userNo) {
        /* 설명. PathVariable 로 넘어온 번호와 일치하는 회원 한명 추출 */
    ...
    public ResponseEntity<?> remmoveUser(@PathVariable int userNo) {
        /* 설명. PathVariable 로 넘어온 번호와 일치하는 회원 한명 추출 */
        UserDTO foundUser = users.stream().filter(user -> user.getNo() == userNo)
                .collect(Collectors.toList())
                .get(0);

        /* 설명. ArrayList 에서 제공하는 remove 는 인덱스 대신 자신이 관리하는 객체를 찾아서 지워주기도 한다. */
